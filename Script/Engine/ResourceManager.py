...
class ResourceManager:
	...
	@staticmethod
	def load_image(path: str, isglobal = False) -> pygame.Surface:
		"""โหลดและ cache รูปภาพ"""
		if isglobal:
			if path not in ResourceManager.global_cache:
				ResourceManager.global_cache[path] = pygame.image.load(path)
			return ResourceManager.global_cache[path]
		else:
			if path not in ResourceManager.cache:
				ResourceManager.cache[path] = pygame.image.load(path)
